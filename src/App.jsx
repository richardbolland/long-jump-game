    ...
    // --- RIVE IMPORTS ---
    // Use the 'Lite' version to avoid WebGL context limits on Firefox
import { useRive, Layout, Fit, Alignment } from '@rive-app/react-canvas';
    // --- CRAZY GAMES SDK IMPORT ---
    import { 
      initSDK, gameStart, gameStop, happyTime, requestAd, requestRewardAd, getUser, promptLogin,
      gameplayStart, gameplayStop, attachAuthListener
    } from './CrazyGamesSDK';
    ...
    // --- CONSTANTS & CONFIG ---
    const GRID_ROWS = 5;
    const GRID_COLS_STANDARD = 150; // Changed from 50 to 150
    const GRID_COLS_MAZE = 150;     // Changed from 50 to 150 
    ...
    const generateRandomBonuses = (seedDateString, cols, mode) => {
      ...
      // 1. Generate Distributed Bonuses (Column Walk)
      // Instead of random scattering, we check every column.
      for (let c = 1; c < cols; c++) {
          // MAZE EXCLUSIVE: Skip col 14 & 29 (Reserved for specific pattern)
          if (mode === 'maze' && (c === 14 || c === 29)) continue;

          // 30% Chance per column to have a bonus
          if (rand() < 0.3) {
  ...
  // 2. FIREFOX FALLBACK: Show SVG to prevent crash
  if (isFirefox) {
    return (
  ...
  // 3. OTHER BROWSERS: Show the Rive Animation
...
// 1. The Heavy Rive Component (Standard Browsers Only)
const RiveFan = ({ trigger }) => {
  ...
  useEffect(() => {
    if (rive && trigger) {
      ...
      const triggerInput = inputs.find(i => i.name === 'Trigger 1');
      if (triggerInput) triggerInput.fire();
    }
  }, [rive, trigger]);

  return <RiveComponent className="w-16 h-16" />;
};

// 3. The Safe Fan Wrapper
const Fan = ({ trigger }) => {
  const isFirefox = typeof navigator !== 'undefined' && /firefox/i.test(navigator.userAgent);

  // Generate random fan image AND random animation speed ONCE on mount
  const [fanConfig] = useState(() => {
    ...
    return {
      ...
      // Random speed between 2s and 4s for the subtle "breathing" motion
      duration: .5 + Math.random() * 1 
    };
  });

  // FIREFOX: Show Static SVG with Custom Animation
  if (isFirefox) {
    return (
      // We set specific W/H (16 = 4rem = ~64px) to match the Rive fan size
      <div 
        className={`flex items-center justify-center w-8 h-8 transition-transform duration-300 ${trigger ? 'animate-bounce' : 'animate-float-y'}`}
        style={{ 
            animationDuration: trigger ? '1s' : `${fanConfig.duration}s` // Jump fast on trigger, float slow otherwise
        }}
      >
        <img 
          src={fanConfig.src} 
  ...
  // OTHER BROWSERS: Show Rive Animation
  return <RiveFan trigger={trigger} />;
};
    ...
    const DailyLeaderboard = ({ highlightName, user, onRankFound, lastUpdated, initialMode = 'standard', onParCalculated, theme }) => {
      ...
      const fetchData = async () => {
        // 1. Debug Log: See if this function is actually running
        console.log("Leaderboard: Fetching data...", { user: user ? user.uid : "No User", view, modeFilter });

        // 2. Handle Missing User (Don't just return silently!)
        if (!user) {
            console.log("Leaderboard: Waiting for Firebase Auth...");
            // Keep loading true, because we are technically waiting.
            // But if this persists, we know Auth is the issue.
            return;
        }
        ...
        // Note: Keep your existing startOfWeekDate / startOfMonthDate logic here
        ...
        try {
            console.log("Leaderboard: Querying Firestore..."); // Debug Log
            const q = query(scoresRef, ...constraints);
            const snapshot = await getDocs(q);
            
            console.log(`Leaderboard: Found ${snapshot.size} docs`); // Debug Log
            ...
            // ... (Your existing Weekly/Monthly aggregation logic) ...
        ...
        setError(err.message);
            setLoading(false); // <--- IMPORTANT: Turn off loading if error occurs
        }
    };
    ...
    export default function App() {
      ...
      const [profanityList, setProfanityList] = useState(new Set());
    ...
    // --- INIT SDK & USER ---
      useEffect(() => {
          let authCleanup = null; // Variable to hold the CrazyGames auth listener cleanup
          let firebaseUnsub = null; // Variable to hold the Firebase auth listener cleanup

          // 1. Initialize CrazyGames SDK (Independent)
          const initCG = async () => {
              await initSDK();
              
              // --- CRAZYGAMES AUTH LISTENER ---
              authCleanup = attachAuthListener((user) => {
                  if (user) {
                      console.log("SDK: User Logged In! Refreshing to load account data...");
          ...
          // THIS WAS ALSO MISSING - Needed to tell the leaderboard "User is ready"
          firebaseUnsub = onAuthStateChanged(auth, (u) => {
              console.log("Firebase Auth State Changed:", u ? "Logged In" : "Logged Out");
              setUser(u); // Update the global 'user' state which DailyLeaderboard watches
          }); 
    ...
    // --- LOAD PROFANITY FILTER ---
      useEffect(() => {
        const loadProfanityList = async () => {
          try {
            const response = await fetch('./profanityfilterlist.csv');
            ...
            setProfanityList(new Set(words)); 
            console.log(`Loaded ${words.length} blocked words.`);
        } catch (e) {
    ...
    loadProfanityList();
      ...
      // --- CRAZYGAMES PAUSE MANAGER ---
      useEffect(() => {
        // Only run this logic if we are actually playing
        if (gameState !== 'playing' || isFinishing) return;

        // Check if ANY modal or menu is currently open
        const isPaused = 
        showRules || 
        showSettings || 
        showChangelog || 
        ...
        if (isPaused) {
          gameplayStop();
          console.log("SDK: Gameplay Stopped (Menu Open)");
      } else {
          gameplayStart();
          console.log("SDK: Gameplay Resumed");
      }
    }, [
        gameState, 
        isFinishing,
    ...
    // --- PROFANITY HELPER (UPDATED) ---
    const containsProfanity = (text) => {
      if (!text) return false;
      const upperText = text.toUpperCase();

          // 1. Split text into distinct words
          // We split by anything that isn't a letter (spaces, numbers, punctuation)
          // This ensures "Assassin" stays as "ASSASSIN", but "Ass.Hole" becomes "ASS", "HOLE"
      const words = upperText.split(/[^A-Z]+/); 

          // 2. Check if any EXACT word matches the ban list
          // "ASSASSIN" is not in the list -> OK
          // "ASS" is in the list -> BLOCKED
    ...
    const handleStartGame = (mode) => {
        ...
        if (containsProfanity(playerName)) {
            setMessage({ text: "Please choose a polite username to play.", type: "error" });
        ...
        // 3. SHOW AD THEN START (Or start immediately if local)
        requestAd('midgame', {
            onAdFinished: runGame,
            onAdError: runGame // Fail-safe: play anyway if ad crashes
        });
    };
    ...
    // --- NEW: UNDO AD HANDLER ---
    const handleWatchAdToUndo = () => {
          // 1. Request a Rewarded Ad
      requestRewardAd(() => {
              // 2. If they watch the whole video, perform the Undo
          confirmUndo();
              // The confirmUndo function already handles closing the modal 
              // and updating the state, so we don't need to do anything else here.
      });
    };
    ...
    const handleConfirmSubmit = () => {
          // 1. Close Modal & Play Sound
      setShowConfirmSubmit(false);
      handleFeedback('fanfare');

          // 2. INSTANTLY Switch to Game Over Screen
          // We skip 'setIsFinishing' and 'setIsSubmittingScore' to avoid loading spinners/delays
      setGameState('gameOver');

          // 3. Save Score in the Background (Fire & Forget)
          // This runs parallel to the screen switch, so the user doesn't wait
      const saveToDb = async () => {
          ...
          if (user && !isTestUser) { 
              try {
                  ...
                  console.log("✅ Score saved in background");
              } catch (e) { 
    ...
    const failValidation = (msg) => { 
      setMessage({ text: msg, type: "error", invalidWord: null }); 
        ...
        return (
          <div className="fixed inset-0 z-50 font-sans flex flex-col lg:flex-row items-center justify-start lg:justify-center p-4 gap-8 lg:gap-12 overflow-y-auto transition-colors duration-300" style={{ backgroundColor: theme.background, color: theme.textMain }}>
            ...
            <div className="w-full max-w-md flex flex-col items-center text-center space-y-12 animate-fade-in justify-start pt-0 lg:pt-0 lg:justify-center relative lg:flex-1 lg:h-[600px] lg:self-stretch">
                ...
                <div className="w-full max-w-xs space-y-4">
                    {cgUser ? (
                        ...
                        <div className="space-y-3">
                            <div className="group">
                                <label className="block text-xs font-bold tracking-widest uppercase text-left pl-1 mb-1 transition-colors" style={{ color: theme.textSub }}>
                                    Guest Name
                                </label>
                ...
                // 1. ALWAYS update the text so the user can type freely
                                        setPlayerName(input);

                // 2. Check for profanity to toggle the Red Error State
                                    ...
                                    maxLength={12} 
                                    className="w-full border-2 outline-none px-6 py-3 rounded-2xl text-xl font-bold text-center shadow-sm transition-all" 
                                    style={{ 
                                        backgroundColor: theme.modalBg, 
                                        color: theme.textMain, 
                // If nameError is true, border is RED
                                        borderColor: nameError ? '#ef4444' : 'transparent' 
                                    }} 
                                    onFocus={(e) => !nameError && (e.target.style.borderColor = theme.accentPrimary)} 
                                    onBlur={(e) => !nameError && (e.target.style.borderColor = 'transparent')} 
                                />

        {/* Error Message Display */}
                                {nameError && (
                                    <div className="text-red-500 text-xs font-bold mt-2 animate-pulse text-center">
         ...
         {showSettings && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/40 backdrop-blur-sm animate-fade-in p-4 pointer-events-auto">
              <div className="rounded-2xl p-6 shadow-2xl max-w-sm w-full border max-h-[85vh] overflow-y-auto custom-scrollbar bg-white flex flex-col" style={{ backgroundColor: theme.modalBg }}>
                          ...
                          <input 
                              ...
                              onChange={(e) => {
                                  setPlayerName(e.target.value);
                                      // We don't need to set nameError state here anymore, 
                                      // we calculate it directly in the render below.
                              }} 
                              maxLength={12} 
                              className="w-full border-2 px-4 py-3 rounded-xl font-bold outline-none transition-all bg-gray-50" 
                              style={{ 
                                      // Check profanity live during render
                                  borderColor: containsProfanity(playerName) ? '#ef4444' : theme.boardLines, 
                                  color: theme.textMain 
                              }} 
                          />
                          
                              {/* LIVE ERROR MESSAGE */}
                          {containsProfanity(playerName) && (
                              <div className="text-red-500 text-xs font-bold mt-2 animate-pulse">
                                  Username not allowed
                              </div>
                              )}
                      </div>
          ...
          <div className="flex justify-center pt-2">
              <a href="https://www.richardbolland.co.za/privacy-policy" target="_blank" rel="noopener noreferrer" className="text-[10px] opacity-50 hover:opacity-100 font-bold uppercase tracking-widest transition-opacity" style={{ color: theme.textSub }}>
                  Privacy Policy
                      ...
                      {/* NEW SAVE BUTTON (Replaces Close Logic) */}
      <div className="mt-6 pt-4 border-t" style={{ borderColor: theme.boardLines }}>
          <button 
              onClick={() => setShowSettings(false)} 
                              disabled={containsProfanity(playerName)} // Disable if bad word
                              className={`w-full py-4 rounded-xl font-black uppercase tracking-widest text-sm transition-all shadow-lg 
                                  ...
                                  style={!containsProfanity(playerName) ? { backgroundColor: theme.accentPrimary } : {}}
                              >
                                  {containsProfanity(playerName) ? "Fix Username to Save" : "Save Settings"}
                              </button>
                          </div>
    ...
    {showUndoModal && (
      <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/40 backdrop-blur-sm animate-fade-in p-4">
          <div className="rounded-2xl p-6 shadow-2xl max-w-sm w-full border text-center" style={{ backgroundColor: theme.modalBg, borderColor: theme.boardLines }}>
              ...
              <h3 className="text-xl font-black mb-2 uppercase" style={{ color: theme.textMain }}>Undo Move?</h3>
              <p className="mb-6 text-sm" style={{ color: theme.textSub }}>
                  Watch a short video to undo your last move. <br/><br/>
                  <span className="text-xs opacity-70">(One use per turn)</span>
              </p>
              <div className="flex gap-3">
                  <button onClick={() => setShowUndoModal(false)} className="flex-1 py-3 rounded-xl font-bold hover:opacity-80 transition-colors" style={{ backgroundColor: theme.boardLines, color: theme.textSub }}>Cancel</button>
                          ...
                          {/* UPDATED BUTTON: Calls handleWatchAdToUndo */}
                  <button 
                      onClick={handleWatchAdToUndo} 
                      className="flex-1 py-3 rounded-xl font-bold text-white shadow-lg transition-all bg-blue-500 hover:bg-blue-600 flex items-center justify-center gap-2"
                  >
                   <Play size={16} fill="currentColor" /> Watch to Undo
               </button>
           </div>
       </div>
    </div>
      ...
      <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/40 backdrop-blur-sm animate-fade-in p-4 pointer-events-auto">
          <div className="rounded-2xl p-6 shadow-2xl max-w-sm w-full border max-h-[85vh] overflow-y-auto custom-scrollbar bg-white flex flex-col" style={{ backgroundColor: theme.modalBg }}>
              ...
              <div className="space-y-6 flex-1">
                  ...
                  <div>
                      ...
                      {containsProfanity(playerName) && (
                          ...
                          {/* 2. SAVE BUTTON (Moved Here) */}
                  <div>
                      <button 
                          onClick={() => setShowSettings(false)} 
                          disabled={containsProfanity(playerName)} 
                          className={`w-full py-3 rounded-xl font-black uppercase tracking-widest text-sm transition-all shadow-md 
                                      ${containsProfanity(playerName) 
                                          ? 'bg-gray-300 text-gray-500 cursor-not-allowed opacity-50' 
                                          : 'hover:scale-105 active:scale-100 text-white'
                                      }`}
                                      style={!containsProfanity(playerName) ? { backgroundColor: theme.accentPrimary } : {}}
                                  >
                                      {containsProfanity(playerName) ? "Fix Name" : "Save & Close"}
                                  </button>
                              </div>

                          {/* Divider */}
                              <div className="border-t" style={{ borderColor: theme.boardLines }}></div>
              ...
              <div className="pt-4 border-t" style={{ borderColor: theme.boardLines }}>
                  ...
                  <textarea value={suggestionText} onChange={(e) => setSuggestionText(e.target.value)} className="w-full border-2 px-4 py-3 rounded-xl font-medium text-sm outline-none transition-all resize-none h-24 bg-gray-50" style={{ borderColor: theme.boardLines, color: theme.textMain }} placeholder="Ideas..." />
                      <button onClick={handleSubmitSuggestion} disabled={suggestionStatus !== 'idle' || !suggestionText.trim()} className={`w-full mt-2 py-3 rounded-xl font-bold uppercase tracking-widest text-xs transition-all shadow-sm bg-gray-500 text-white`}>Submit Suggestion</button>
                  </div>
        ...
        <div className="hidden lg:flex justify-between items-start w-full">
         ...
         <div className="flex flex-col items-start w-32"> 
            ...
            {dailyLeader && (<div className="flex items-center gap-1 mt-0.5 text-[10px] animate-fade-in whitespace-nowrap"><Crown size={10} className="text-yellow-500 fill-yellow-500" /><span className="font-bold opacity-80" style={{ color: theme.textSub }}>LEADER</span><span className="font-bold truncate max-w-[80px]" style={{ color: theme.textMain }}>{dailyLeader.name}</span><span className="font-mono opacity-60" style={{ color: theme.textSub }}>{dailyLeader.score}m</span></div>)}
                 ...
                 {/* Center: Logo (Button Removed) */}
            <div className="flex flex-col items-center flex-1 -mt-2">
                <div className="w-64">
                    <RiveLogo />
                </div>
                    {/* Empty div to keep spacing if needed, or just leave empty */}
                <div className="mt-2 h-6"></div>
            </div>

                 {/* Right: Controls (ONLY BIG ICONS NOW) */}
            <div className="flex flex-col items-end w-32 gap-1">
              ...
              <div className="flex gap-2 mt-1">
                <button onClick={() => setShowRules(true)} className="w-9 h-9 flex items-center justify-center rounded-lg bg-black/5 text-gray-500 active:bg-black/10 active:scale-95 transition-all shadow-sm hover:bg-black/10" style={{ color: theme.textSub }} title="Rules">
                    <Info size={20} />
                </button>
                <button onClick={() => setShowSettings(true)} className="w-9 h-9 flex items-center justify-center rounded-lg bg-black/5 text-gray-500 active:bg-black/10 active:scale-95 transition-all shadow-sm hover:bg-black/10" style={{ color: theme.textSub }} title="Settings">
                    <Settings size={20} />
                </button>
                <button onClick={() => setShowExitConfirmation(true)} className="w-9 h-9 flex items-center justify-center rounded-lg bg-red-50 text-red-500 active:bg-red-100 active:scale-95 transition-all shadow-sm border border-red-100 hover:bg-red-100" title="Exit Game">
                    <LogOut size={20} />
                </button>
            </div>
        </div>
        ...
        <div ref={scrollContainerRef} className={`w-full overflow-x-auto overflow-y-visible custom-scrollbar flex items-center landscape:h-full no-scrollbar relative z-0 ${isDraggingBoard ? 'cursor-grabbing' : 'cursor-grab'} py-4`} style={{ userSelect: 'none' }}>
            <div className="relative p-1 mx-4 pt-16 pb-16" style={{ width: 'fit-content', minWidth: 'fit-content' }} id="board-capture-area">
                ...
                <div className="relative">
                    {/* TOP FANS - Hidden on Firefox to prevent crashes */}
                    ...
                    <div className="relative z-10 grid gap-0 rounded-xl shadow-inner border-4" style={{ gridTemplateColumns: `repeat(${grid[0].length}, ${CELL_SIZE}px)`, gridTemplateRows: `repeat(${GRID_ROWS}, ${CELL_SIZE}px)`, backgroundColor: theme.boardBackground, borderColor: theme.boardLines }}>
                        {Array(GRID_ROWS).fill(0).map((_, r) => (Array(grid[0].length).fill(0).map((_, c) => {
                            ...
                            // Blue Box logic: Hidden when dragging
                            const isSelected = !isDraggingTile && selectedGridSpot && selectedGridSpot.r === r && selectedGridSpot.c === c;
                            ...
                            return (
                                <div 
                                    ...
                                    style={{
                                        ...(isLocked 
                                            ? { backgroundColor: isCompleted ? theme.overlayComplete : theme.tileLocked, color: theme.tileLockedText, boxShadow: '0 2px 0 rgba(0,0,0,0.1)', animationDelay: isFinishing ? `${c * 50}ms` : '0ms' } 
                                            : (content?.type === 'temp' 
                                                ? { backgroundColor: content.isWild ? theme.tileWildPlaced : theme.tileTemp, boxShadow: '0 4px 0 rgba(0,0,0,0.1)' } 
                                                : (obstacle 
                                                    ? { backgroundColor: theme.tileObstacle, borderRadius: '8px', backgroundImage: 'repeating-linear-gradient(45deg, transparent, transparent 5px, rgba(255,255,255,0.05) 5px, rgba(255,255,255,0.05) 10px)' } 
                                                    : (gameMode === 'maze' && (c === 14 || c === 29) ? { backgroundColor: '#fef08a', borderLeft: '2px solid #eab308', borderRight: '2px solid #eab308' } : {})
                                                    )
                                                )
                                            )
                                    }}
                                    ...
                                    {!content && isStartZone && (<div className="absolute inset-0 flex items-center justify-center opacity-30 pointer-events-none z-10"><ChevronRight size={20} style={{ color: theme.startArrow }} /></div>)}
                                        ...
                                        {!content && bonus && !obstacle && !isStarSpot && (<div className="absolute inset-0 flex flex-col items-center justify-center opacity-80 pointer-events-none z-10"><Star className="w-6 h-6" fill={theme.starGold} stroke={theme.starGold} /><span className="text-[16px] font-black mt-[-2px]" style={{ color: '#000000' }}>+{bonus.val}</span></div>)}
                                            {!content && isStarSpot && !obstacle && (<div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none z-10 animate-pulse"><Sparkles className="w-7 h-7" fill={theme.starPurple} stroke={theme.starPurple} /></div>)}
                                                ...
                                                {content && (
                                                    <div 
                                                        draggable={content.type === 'temp' && !isFinishing && gameState === 'playing'} 
                                                        onDragStart={(e) => handleDragStart(e, { type: 'board', r, c, letter: content.letter })} 
                                                        onDragEnd={handleDragEnd} 
                                                        className={`w-full h-full flex items-center justify-center ${content.type === 'temp' ? 'cursor-grab active:cursor-grabbing' : 'cursor-default'}`}
                                                    >
                                                        <span className="text-2xl font-bold select-none font-mono">{content.letter}</span>
                                                        {content.isWild && <Sparkles size={12} className="absolute top-0.5 right-0.5 text-purple-500 fill-purple-500" />}
                                                        </div>
                                                        )}
                                            </div>
                                            );
                    ...
                    {/* BOTTOM FANS - Hidden on Firefox to prevent crashes */}
    <div className="absolute top-0 left-0 w-full h-full z-30 pointer-events-none">
    ...
    <div className="w-full max-w-4xl px-4 pb-2 md:pb-6 landscape:pb-2 flex flex-col items-center shrink-0 relative z-50">
        ...
        <div onDragOver={handleDragOver} onDrop={handleDropOnHand} className="flex flex-col items-center justify-center p-2 md:p-3 rounded-2xl shadow-inner w-full transition-colors border-4 border-white/40 backdrop-blur-sm" style={{ backgroundColor: theme.handBg }}>
            ...
            .animate-float-y {animation-name: float-y; animation-iteration-count: infinite; animation-timing-function: ease-in-out;}
